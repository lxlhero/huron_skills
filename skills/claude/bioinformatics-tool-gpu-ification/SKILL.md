...
68. **SuSiE GPU local optima divergence when L ≈ L_true on real LD data (known limitation)**: When running `susie_gpu_numpy(X, y, L)` with L equal to (or close to) the true number of effects on real correlated genomic data, GPU and CPU may converge to different local optima. Confirmed on 1000G chr22 data (n=2504, p=2115, L=L_true=10): GPU sigma2=1.0826 vs CPU sigma2=1.054 (ratio=1.027), PIP r=0.87, CS count mismatch (9 vs 8). L=3,5,7 all pass on the same data (sigma2 ratio ≤ 1.0005). Root cause: SuSiE IBSS is non-convex. CPU BLAS and GPU cuBLAS float64 rounding differences in `XtX @ b_{-l}` accumulate across L effects × iterations, pushing the two solvers into different basins. Stricter convergence tol (1e-4 vs 1e-3) does NOT fix it — both are genuine local optima (confirmed: GPU niter=12 at tol=1e-4 still gives sigma2=1.082566). CPU finds the better solution (lower sigma2). Likely fix (not yet implemented): switch convergence criterion from Δα < tol to ΔELBO < tol, matching CPU susieR exactly. Workaround for now: when L equals the expected number of true effects and real LD data is used, run both CPU and GPU, compare ELBO; if GPU ELBO is lower than CPU, fall back to CPU result. Diagnostic signal: sigma2_ratio > 1.01 with L=L_true.
